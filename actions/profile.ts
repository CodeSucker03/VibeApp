...
export async function saveProfile(
  ...
  try {
    ...
    const normalizedExperienceLevel =
      data.experienceLevel && experienceLevelMap[data.experienceLevel]
        ? experienceLevelMap[data.experienceLevel]
    ...
    const normalizedRemotePreference =
      data.remotePreference && remotePreferenceMap[data.remotePreference]
        ? remotePreferenceMap[data.remotePreference]
        : data.remotePreference || null;

    const workAuthMap: Record<string, string> = {
      Citizen: "citizen",
      "Permanent Resident": "permanent_resident",
      "Work Visa (H1B)": "work_visa_h1b",
      "Work Visa (Other)": "work_visa_other",
      "Student Visa (OPT/CPT)": "student_visa_opt_cpt",
      "Requires Sponsorship": "requires_sponsorship",
    };
    const normalizedWorkAuth =
      data.workAuth && workAuthMap[data.workAuth]
        ? workAuthMap[data.workAuth]
        : data.workAuth || null;
    ...
    const { isComplete } = calculateCompletion({
      ...
      experience_level: normalizedExperienceLevel || null,
        ...
        [
          {
            ...
            remote_preference: normalizedRemotePreference,
            ...
            work_authorization: normalizedWorkAuth,
          },
        ],
        { onConflict: "id" },
      )
